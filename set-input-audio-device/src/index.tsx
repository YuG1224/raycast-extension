...
export default function main() {
  ...
  function getInputAudioDevices() {
    const script = `
      on run
        ...
        do shell script theSwitch & " -f json -t input -a"
      end run
    `;
  ...
  function getCurrentInputAudioDevice() {
    const script = `
      on run
        set theSwitch to "/opt/homebrew/bin/SwitchAudioSource"
        do shell script theSwitch & " -f json -t input -c"
      end run
    `;
    const { stdout } = execaSync("osascript", ["-e", script]);
    return stdout.split("\r").map((v) => JSON.parse(v));
  ...
  function setInputAudioDevice(v: AudioSource) {
    const script = `
      on run
        set theSwitch to "/opt/homebrew/bin/SwitchAudioSource"
        do shell script theSwitch & " -t input -i ${v.id}"
      end run
    `;
    const { stdout } = execaSync("osascript", ["-e", script]);
            ...
            <ActionPanel>
              <Action
                ...
                onAction={() => {
                  ...
                  showHUD(`Active input audio device set to ${v.name}`);
                }}
              />
            </ActionPanel>
          }
